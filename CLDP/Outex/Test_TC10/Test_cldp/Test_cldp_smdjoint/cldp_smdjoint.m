...
function CLBP_SMDjoint = cldp_smdjoint(varargin) % image,radius,neighbors,mapping,mode)
...
CLBP_SMDjoint=zeros(dy+1,dx+1);
CLBP_SMDjoint=bins^2*CLBP_S+bins*CLBP_M+CLBP_D;
...
%     CLBP_D=hist(CLBP_D(:),0:(bins-1));
    CLBP_SMDjoint=hist(CLBP_SMDjoint(:),0:(bins^3-1));
...
%         CLBP_D=CLBP_D/sum(CLBP_D);
        CLBP_SMDjoint=CLBP_SMDjoint/sum(CLBP_SMDjoint);
    end
else
