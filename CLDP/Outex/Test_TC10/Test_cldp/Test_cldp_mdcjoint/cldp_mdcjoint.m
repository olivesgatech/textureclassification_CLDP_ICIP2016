...
function CLBP_MDCjoint = cldp_mdcjoint(varargin) % image,radius,neighbors,mapping,mode)
...
CLBP_MDCjoint=zeros(dy+1,dx+1);
CLBP_MDCjoint=bins*CLBP_M+CLBP_D+bins^2*CLBP_C;
...
%     CLBP_D=hist(CLBP_D(:),0:(bins-1));
    CLBP_MDCjoint=hist(CLBP_MDCjoint(:),0:(2*bins^2-1));
...
%         CLBP_D=CLBP_D/sum(CLBP_D);
        CLBP_MDCjoint=CLBP_MDCjoint/sum(CLBP_MDCjoint);
    end
else
