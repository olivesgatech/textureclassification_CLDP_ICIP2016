...
function [CLBP_M, CLBP_DCjoint] = cldp_m_dcjoint(varargin) % image,radius,neighbors,mapping,mode)
...
CLBP_DCjoint=zeros(dy+1,dx+1);
CLBP_DCjoint=CLBP_D+bins*CLBP_C;
...
%     CLBP_D=hist(CLBP_D(:),0:(bins-1));
    CLBP_DCjoint=hist(CLBP_DCjoint(:),0:(2*bins-1));
...
%         CLBP_D=CLBP_D/sum(CLBP_D);
        CLBP_DCjoint=CLBP_DCjoint/sum(CLBP_DCjoint);
    end
else
