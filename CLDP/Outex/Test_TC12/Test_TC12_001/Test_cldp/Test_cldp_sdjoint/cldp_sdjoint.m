...
function CLBP_SDjoint = cldp_sdjoint(varargin) % image,radius,neighbors,mapping,mode)
...
CLBP_SDjoint=zeros(dy+1,dx+1);
CLBP_SDjoint=bins*CLBP_S+CLBP_D;
...
%     CLBP_D=hist(CLBP_D(:),0:(bins-1));
    CLBP_SDjoint=hist(CLBP_SDjoint(:),0:(bins^2-1));
...
%         CLBP_D=CLBP_D/sum(CLBP_D);
        CLBP_SDjoint=CLBP_SDjoint/sum(CLBP_SDjoint);
    end
else
