...
function CLBP_SMjoint = clbp_smjoint(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.2
...
CLBP_SMjoint=zeros(dy+1,dx+1);
CLBP_SMjoint=bins*CLBP_S+CLBP_M;

if (strcmp(mode,'h') || strcmp(mode,'hist') || strcmp(mode,'nh'))
    % Return with LBP histogram if mode equals 'hist'.
%     CLBP_S=hist(CLBP_S(:),0:(bins-1));
%     CLBP_M=hist(CLBP_M(:),0:(bins-1));
    CLBP_SMjoint=hist(CLBP_SMjoint(:),0:(bins*bins-1));
    if (strcmp(mode,'nh'))
%         CLBP_S=CLBP_S/sum(CLBP_S);
%         CLBP_M=CLBP_M/sum(CLBP_M);
        CLBP_SMjoint=CLBP_SMjoint/sum(CLBP_SMjoint);
...
% %         CLBP_S=uint8(CLBP_S);
% %         CLBP_M=uint8(CLBP_M);
%         CLBP_SMjoint=uint8(CLBP_SMjoint);
%     elseif ((bins-1)<=intmax('uint16'))
% %         CLBP_S=uint16(CLBP_S);
% %         CLBP_M=uint16(CLBP_M);
%         CLBP_SMjoint=uint16(CLBP_SMjoint);
% 
%     else
% %         CLBP_S=uint32(CLBP_S);
% %         CLBP_M=uint32(CLBP_M);
%         CLBP_SMjoint=uint32(CLBP_SMjoint);
