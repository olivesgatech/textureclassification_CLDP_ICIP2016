...
function [CLBP_S CLBP_MCjoint] = clbp_s_mcjoint(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.2
...
CLBP_MCjoint=zeros(dy+1,dx+1);
CLBP_MCjoint=bins*CLBP_C+CLBP_M;
...
%     CLBP_M=hist(CLBP_M(:),0:(bins-1));
    CLBP_MCjoint=hist(CLBP_MCjoint(:),0:(2*bins-1));
    if (strcmp(mode,'nh'))
        CLBP_S=CLBP_S/sum(CLBP_S);
%         CLBP_M=CLBP_M/sum(CLBP_M);
        CLBP_MCjoint=CLBP_MCjoint/sum(CLBP_MCjoint);
    end
else
...
%     if ((bins-1)<=intmax('uint8'))
        CLBP_S=uint8(CLBP_S);
%         CLBP_M=uint8(CLBP_M);
        CLBP_MCjoint=uint8(CLBP_MCjoint);
%     elseif ((bins-1)<=intmax('uint16'))
        CLBP_S=uint16(CLBP_S);
%         CLBP_M=uint16(CLBP_M);
        CLBP_MCjoint=uint16(CLBP_MCjoint);
%     else
        CLBP_S=uint32(CLBP_S);
%         CLBP_M=uint32(CLBP_M);
        CLBP_MCjoint=uint32(CLBP_MCjoint);
