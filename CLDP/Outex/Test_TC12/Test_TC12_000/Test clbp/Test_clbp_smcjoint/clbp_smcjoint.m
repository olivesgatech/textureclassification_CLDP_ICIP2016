...
function CLBP_SMCjoint = clbp_smcjoint(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.2
...
CLBP_SMCjoint=zeros(dy+1,dx+1);
CLBP_SMCjoint=bins*CLBP_S+CLBP_M+bins^2*CLBP_C;

if (strcmp(mode,'h') || strcmp(mode,'hist') || strcmp(mode,'nh'))
    % Return with LBP histogram if mode equals 'hist'.
%     CLBP_S=hist(CLBP_S(:),0:(bins-1));
%     CLBP_M=hist(CLBP_M(:),0:(bins-1));
    CLBP_SMCjoint=hist(CLBP_SMCjoint(:),0:(2*bins*bins-1));
    if (strcmp(mode,'nh'))
...
% %         CLBP_S=uint8(CLBP_S);
% %         CLBP_M=uint8(CLBP_M);
%         CLBP_SMCjoint=uint8(CLBP_SMCjoint);
%     elseif ((bins-1)<=intmax('uint16'))
% %         CLBP_S=uint16(CLBP_S);
% %         CLBP_M=uint16(CLBP_M);
%         CLBP_SMCjoint=uint16(CLBP_SMCjoint);
% 
%     else
% %         CLBP_S=uint32(CLBP_S);
% %         CLBP_M=uint32(CLBP_M);
%         CLBP_SMCjoint=uint32(CLBP_SMCjoint);
