...
function CLBP_MDjoint = cldp_mdjoint(varargin) % image,radius,neighbors,mapping,mode)
...
CLBP_MDjoint=zeros(dy+1,dx+1);
CLBP_MDjoint=bins*CLBP_M+CLBP_D;
...
%     CLBP_D=hist(CLBP_D(:),0:(bins-1));
    CLBP_MDjoint=hist(CLBP_MDjoint(:),0:(bins^2-1));
...
%         CLBP_D=CLBP_D/sum(CLBP_D);
        CLBP_MDjoint=CLBP_MDjoint/sum(CLBP_MDjoint);
    end
else
